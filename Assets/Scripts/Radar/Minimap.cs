...
public class Minimap : MonoBehaviour
{
	...
	void Start()
    {
        ...
        if (Radar == null)
        {
            ...
            Radar = playerObject.GetComponentInChildren<Radar>();
    ...
    {
        mapWidth = Screen.width * 1f;   // 20% of width
        mapHeight = Screen.height * 1f;
        ...
        mapCenter = new Vector2((mapStart.x + mapSize.x / 2)-5, (mapStart.y + mapSize.y / 2)+75);
	...
	void OnGUI()
	{
		...
		//if ((DateTime.Now - beaconCheckTime).TotalMilliseconds > 0)
		{
			...
			{
				Unity.Netcode.NetworkObject no = b.GetComponent<Unity.Netcode.NetworkObject>();
				if (no == null || (b.gameObject.tag != "Player" || !no.IsLocalPlayer))
				{
